...
| `API_KEYS_ENABLED` | `true` | Set to `false` to turn [API keys](api.md#api-keys) off for the whole instance, admins included: existing keys stop working, the `/api/api-keys` endpoints refuse, and the UI hides the key menus. Which users may hold keys is set per user in Settings → Users. |
...
| `AUTH_RATE_LIMIT` | `10/minute` | Per-IP throttle applied to the credential-checking endpoints (`/api/auth/login`, `/api/auth/setup`, `/api/auth/guest-login`) and to failed `X-API-Key` attempts on any endpoint. Exceeding it returns `429`. Uses a [`limits`](https://limits.readthedocs.io/en/stable/quickstart.html#rate-limit-string-notation) string like `20/minute` or `100/hour`. See [Security hardening](security.md). |
