...
**Session times are anchored to the schedule's timezone.** A schedule's `days` are *local* weekday indices while `time_utc` is a UTC wall clock, so the definition also carries an optional IANA `timezone` (captured from the browser on save, validated on write). The feed resolves each session's local date and time to a real UTC instant through that zone, which is what places a "Tuesday 20:00 America/Los_Angeles" game at `03:00Z` on Wednesday rather than a day early — and re-resolves per session so the offset stays right across a DST transition. Schedules stored before the field existed have no zone to recover and are published unconverted; re-saving the schedule captures it.
