...
- **Full-text search** — Every page of every PDF is indexed with SQLite FTS5. Also finds books by title, author, or publisher, and maps, tokens, audio, and models by filename, folder, or tag. Narrow with `title:`, `author:`, `tag:` and friends — see [Searching your library](#searching-your-library).
...
- **Token editor** — Turn any picture into a VTT-ready token: position the art, pick a frame and output size, and export it. See [Token editor](#token-editor).
...
- **Wiki note templates** — Start a page from a community template, write your own, or upload a Markdown file or `.zip`. Templates belong to the campaign, so downloaded ones are yours to edit. See [`docs/wiki-templates.md`](docs/wiki-templates.md).

### Administration

- **Book restrictions** — Restrict a book, system, or category to *GMs and admins* or *admins only*. Restricted content is hidden outright — from library, search, downloads, favourites, and OPDS — since the title and cover are the spoiler. See [Restricting books](#restricting-books).
- **Community add-ons** — Install community metadata scrapers to fill in system and book details from external sources, reviewing a field-by-field diff before anything is written. See [`docs/addons.md`](docs/addons.md).
- **Themes and light mode** — Light, dark, or system, plus installable colour themes (including a WCAG AAA **High Contrast** palette). Themes are per user, so no admin approval is involved. See [`docs/themes.md`](docs/themes.md).
