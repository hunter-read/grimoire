...
The permission checks, link-graph rebuilding, and slug helpers these handlers
lean on live in ``_wiki_helpers.py``; they are re-exported here so importers of
``.wiki`` are unaffected by that split.
"""
...
from ._wiki_helpers import (
    ...
    rebuild_links,
    rewrite_inbound_titles,
)
from .wikilinks import extract_headings, slugify
