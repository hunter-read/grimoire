    ...
    return (
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <button
          ...
          title={isOwner ? t('schedule.availability.uncancel') : t('schedule.availability.cancelledLabel')}
          aria-label={isOwner ? t('schedule.availability.uncancel') : t('schedule.availability.cancelledLabel')}
          style={{
...
export default function AvailabilityChart({ availability, userId, isOwner, onSetAvailability, onCancelDate }) {
  ...
  const translatedAvailOptions = AVAIL_OPTIONS.map(o => ({ ...o, label: AVAIL_LABELS[o.value] ?? o.label }))
  ...
  return (
    <div style={{
      ...
      <h4 style={{ fontSize: 14, fontWeight: 600, marginBottom: 16 }}>{t('schedule.availability.title')}</h4>
            ...
            {availability.next_sessions.map(date => {
              ...
              return (
                ...
                >
                  ...
                  <div>{short}</div>
                  {cancelled && <div style={{ fontSize: 11, color: 'var(--danger)', fontWeight: 700 }}>{t('schedule.availability.cancelled')}</div>}
                </th>
              )
            })}
          </tr>
          ...
          {availability.rows.map(row => (
            <tr key={row.user_id}>
              <td style={{ padding: '10px 10px', borderBottom: '1px solid var(--border)', fontWeight: row.is_owner ? 600 : 400 }}>
                {row.username}
                {row.is_owner && <span style={{ marginLeft: 5, fontSize: 11, color: 'var(--gold)' }}>{t('members.gm')}</span>}
                {row.user_id === userId && !row.is_owner && <span style={{ marginLeft: 5, fontSize: 11, color: 'var(--text-muted)' }}>{t('members.you')}</span>}
              </td>
              {availability.next_sessions.map(date => {
